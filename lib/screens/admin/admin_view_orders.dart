...
class _AdminViewOrdersState extends State<AdminViewOrders> {
  ...
  final List<String> _categories = ["All", "pending", "shipped", "refunded"];
  ...
  List<Map<String, dynamic>> _applySearchFilter(
    ...
    return list.where((order) {
      final subject = (order['subject'] as String? ?? '').toLowerCase();
      final status = (order['status'] as String? ?? '').toLowerCase();
      final userId = (order['user_id'] as String? ?? '').toLowerCase();
  ...
  Future<void> _fetchInitialorders() async {
    try {
      ...
      final fetchedOrders = List<Map<String, dynamic>>.from(data);
